    ...
    <div>
        ...
        <div class="flex w-64 flex-col fixed inset-y-0">
            ...
            <div class="flex flex-col flex-grow pt-5 bg-indigo-700 overflow-y-auto">
                ...
                <div class="mt-5 flex-1 flex flex-col">
                    <nav class="flex-1 px-2 pb-4 space-y-1">
                        <a v-for="item in userNavigation" :key="item.name" :href="item.href" :class="[item.current ? 'bg-indigo-800 text-white' : 'text-indigo-100 hover:bg-indigo-600', 'group flex items-center px-2 py-2 text-sm font-medium rounded-md']">
                            <component :is="item.icon" class="mr-3 flex-shrink-0 h-6 w-6 text-indigo-300" aria-hidden="true" />
                            {{ item.name }}
                        </a>
                    </nav>
...
import { BellIcon, CalendarIcon, ChartBarIcon, FolderIcon, HomeIcon, InboxIcon } from '@heroicons/vue/24/outline';
    ...
    data() {
        return {
            ...
            userNavigation: [
                ...
                //{ name: 'Settings', href: '#' },
                { name: 'Sign out',  href: '/admin/logout',  icon: HomeIcon,  current: false },
            ],
            sidebarOpen: false,
            active: false,
        }
